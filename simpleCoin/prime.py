...
SEED, GENERATOR, RMAX, STATE = 0x28d2c378a13f7985f, 0x28d2c378a13f7985f, 0x2f2191d53fd937c2716bb869cfac40240e3042f21d6ca1cf9f09498a496fe7983, None
                                                    
def randinit(seed):
    global STATE
    STATE = ((seed * GENERATOR) ^ 0x245c7bfcc07aff5df56a6c354c6b527e92daf65b06bfb34e1e50e2c1444a48943) % RMAX

def _randrange(a, b,seed):
    global STATE
    if STATE == None:
        randinit(seed)
    r = b - a
...
def randprime_range(a, b, seed, c=None):
    if not c:
        if b > 2**64:
    ...
    while True:
        k = _randrange(a, b,seed) | 1 # ensure it's not even
        if is_probable_prime(k, c):
            return k

def randprime_bits(seed, n=256):
    return randprime_range(2**n, 2**(n+1),seed)
